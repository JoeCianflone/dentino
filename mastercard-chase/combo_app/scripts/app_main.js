...
function is_user() {
	$('.got_user').click(function(){
		$('fieldset#username').fadeIn(500);
...
function validate() {
	...
	function validates(element) {
		...
		if (element.hasClass('check_set_dob')) {
			...
			if (month == 'mm' || day == 'dd' || year == 'yyyy' || month == '' || day == '' || year == '') {
				...
				element.parent().after("<p class='clear error message'>This information is required. Please enter.</p>");
				}
			} else {
				if ($('#month, #day, #year').hasClass('error_field')) {
		...
		if (element.hasClass('check_set_alt_phone')) {
			var field1 = $('#alt_phone1').attr('value');
			var field2 = $('#alt_phone2').attr('value');
			var field3 = $('#alt_phone3').attr('value');

			if (field1 == '' || field2 == '' || field3 == '') {
				if (!$('#alt_phone1, #alt_phone2, #alt_phone3').hasClass('error_field')) {
				$('#alt_phone1, #alt_phone2, #alt_phone3').addClass('error_field');
				element.parent().after("<p class='clear error message'>This information is required. Please enter.</p>");
				}
			} else {
				if ($('#alt_phone1, #alt_phone2, #alt_phone3').hasClass('error_field')) {
				$('#alt_phone1, #alt_phone2, #alt_phone3').removeClass('error_field');
				element.parent().next().remove();
				}
			}
		}
		if (element.hasClass('check_set_phone')) {
			var field1 = $('#phone1').attr('value');
			var field2 = $('#phone2').attr('value');
			var field3 = $('#phone3').attr('value');

			if (field1 == '' || field2 == '' || field3 == '') {
				if (!$('#phone1, #phone2, #phone3').hasClass('error_field')) {
				$('#phone1, #phone2, #phone3').addClass('error_field');
				$('#phone_elem').after("<p class='clear error message'>This information is required. Please enter.</p>");
				}
			} else {
				if ($('#phone1, #phone2, #phone3').hasClass('error_field')) {
				$('#phone1, #phone2, #phone3').removeClass('error_field');
				$('#phone_elem').remove();
				}
			}
		}
	...
	function add_error(element, message) {
		if (!element.hasClass('error_field')) {
			element.addClass('error_field');
			element.parent().after("<p class='clear error message'>"+message+"</p>");
		}
	}
